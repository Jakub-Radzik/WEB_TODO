...
const Row: FC<{text1: string, text2: string}> = ({text1, text2}) => {
    ...
    <PrimaryText color='#000' style={{fontSize: 15, display: 'inline-block'}}>{text1}</PrimaryText>
    <PrimaryText color='#000' style={{fontSize: 15, display: 'inline-block'}}>{text2}</PrimaryText>
    </div>
}
...
const Event: FC<{ event: GoogleEvent }> = ({ event }) =>{
      ...
      () => {
        if(event.start.date && event.end.date){
            ...
            if(event.start.dateTime && event.end.dateTime){
                if(formatDate(event.start.dateTime) === formatDate(event.end.dateTime)){ // same day event
                    return formatDate(event.start.dateTime);
                }
                return `${formatDate(event.start.dateTime)} - ${formatDate(event.end.dateTime)}`;
            }
        }
      },
      [event],
    ...
    return <StyledTaskCard>
        ...
        <TaskContent>
            {event.description && 
                <>
                   <PrimaryText color="#000" style={{fontSize: 20}}>Description</PrimaryText>
        ...
        <Row text1={"Created at"} text2={`${formatDate(event.created)} ${formatTime(event.created)}`}/>
        <Row text1={"Last update"} text2={`${formatDate(event.updated)} ${formatTime(event.updated)}`}/>
